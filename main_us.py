...
async def fetch_multi_timeframe_data(symbol):
    ...
    def get_twelve_data(interval, outputsize):
        ...
        try:
            ...
            if 'values' in response:
                ...
                # Twelve Data 回傳是從新到舊，我們需要反轉成從舊到新 (歷史 -> 現在)
                df = df.sort_index(ascending=True)
                
                # 統一欄位名稱首字母大寫，配合你後面的計算邏輯
                df.rename(columns={'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close', 'volume': 'Volume'}, inplace=True)
                
                return df
            else:
                print(f"⚠️ Twelve Data 回應錯誤 ({interval}): {response}")
                return pd.DataFrame()
        except Exception as e:
            print(f"❌ API 連線失敗: {e}")
            return pd.DataFrame()
    ...
    # 在背景同時抓取三種完美級別的數據
    # 1h 抓 1000 根 (約 1 年多)，1day 抓 500 根 (約 2 年)，1week 抓 260 根 (約 5 年)
    df_1h = await loop.run_in_executor(None, get_twelve_data, "1h", 1000)
    df_1d = await loop.run_in_executor(None, get_twelve_data, "1day", 500)
    df_1wk = await loop.run_in_executor(None, get_twelve_data, "1week", 260)
